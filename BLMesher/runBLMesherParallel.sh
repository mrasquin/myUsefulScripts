...
print_usage()                                                                                  
{
   ...
   echo " Note: soft add +simmodsuite-9.0-140403 if not already done"
...
if [[ $modelfile == *.x_t ]] || [[ $modelfile == *.xmt_txt ]] || [[ $modelfile == *.X_T ]] || [[ $modelfile == *XMT_TXT ]] ; then
	executable=BLMesher-openmpi-O-parasolid-9.0-140403beta
elif [[ $modelfile == *.sat ]] || [[ $modelfile == *.SAT ]] ; then
	executable=BLMesher-openmpi-O-acis-9.0-140403beta
elif [[ $modelfile == *.smd ]] || [[ $modelfile == *.SMD ]] ; then
	executable=BLMesher-openmpi-O-geomsim-9.0-140403beta
else
